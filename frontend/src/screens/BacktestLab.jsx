...
export default function BacktestLab({ feed }) {
  const [form, setForm] = useState({
    ...
    csv_path: DEFAULT_CSV, fixed_lots: 0.02, min_risk_usd: 1, max_risk_usd: 100, book: "cfast_v21",
  });
  ...
  return (
    <div className="flex-1 flex min-h-0">
      <div className="w-[280px] shrink-0 border-r border-[var(--hair)] overflow-y-auto">
        ...
        <div className="px-3 pt-2 text-[9px] mono text-mute">V2.1 only = Hunt fire + setup kill + 1:3</div>
        ...
        <div className="px-3 pb-3 flex flex-col gap-2">
          <button className={`btn active w-full ${running ? "breathe" : ""}`} onClick={() => run("cfast_v21")} disabled={!!running}>
            {running && form.book === "cfast_v21" ? `running ${pctRun.toFixed(0)}%` : "run C-Fast V2.1"}
          </button>
