...
export default function BacktestLab({ feed }) {
  const [form, setForm] = useState({
    ...
    csv_path: DEFAULT_CSV, fixed_lots: 0.02, min_risk_usd: 1, max_risk_usd: 100, book: "liq_join_m1",
  });
  ...
  return (
    <div className="flex-1 flex min-h-0">
      <div className="w-[280px] shrink-0 border-r border-[var(--hair)] overflow-y-auto">
        ...
        <div className="px-3 pt-2 text-[9px] mono text-mute">M1 hold = skip fake close-back</div>
        ...
        <div className="px-3 pb-3 flex flex-col gap-2">
          <button className={`btn active w-full ${running ? "breathe" : ""}`} onClick={() => run("liq_join_m1")} disabled={!!running}>
            {running && form.book === "liq_join_m1" ? `running ${pctRun.toFixed(0)}%` : "run LIQ-JOIN M1 HOLD"}
          </button>
