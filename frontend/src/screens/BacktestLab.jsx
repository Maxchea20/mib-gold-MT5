...
export default function BacktestLab({ feed }) {
  const [form, setForm] = useState({
    ...
    csv_path: DEFAULT_CSV, fixed_lots: 0.02, min_risk_usd: 1, max_risk_usd: 100, book: "liq_m3_v21",
  });
  ...
  return (
    <div className="flex-1 flex min-h-0">
      <div className="w-[280px] shrink-0 border-r border-[var(--hair)] overflow-y-auto">
        ...
        <div className="px-3 pt-2 text-[9px] mono text-mute">LIQ entry + 1:3 TP</div>
        ...
        <div className="px-3 pb-3 flex flex-col gap-2">
          <button className={`btn active w-full ${running ? "breathe" : ""}`} onClick={() => run("liq_m3_v21")} disabled={!!running}>
            {running && form.book === "liq_m3_v21" ? `running ${pctRun.toFixed(0)}%` : "run LIQ M3 + V2.1 1:3"}
          </button>
          <button className="btn w-full" onClick={() => run("liq_join_m3")} disabled={!!running}>run LIQ-JOIN M3</button>
          <button className="btn w-full" onClick={() => run("cfast_v21")} disabled={!!running}>run C-Fast V2.1</button>
