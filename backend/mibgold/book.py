...
class PositionBook:
    """Tracks open hedged layers + closed trades. Identical behaviour live and in backtest."""
    ...
    def open_layer(self, direction: str, entry: float, sl: float, lots: float, risk_usd: float, ts: datetime,
                   votes: dict, consensus: dict, summary: str, session: str, bias: dict, ticket=None) -> Layer:
        ...
        layer = Layer(id=new_id("L"), group_id=gid, layer_number=len(group) + 1, direction=direction,
                      ...
                      agent_votes=votes, consensus=consensus, summary=summary, session=session, bias=bias, ticket=ticket)
    ...
    def on_bar(self, high: float, low: float, close: float, atr: float, ts: datetime, slippage: float = 0.0) -> List[dict]:
        """Update trailing stops then check exits against the bar's range. Stops are hit at worst case."""
        closed = []
        for layer in list(self.layers):
            hit = self.trailing.check_exit(layer, high, low)
            if hit:
                reason, px = hit
                px -= slippage * layer.sign  # adverse slip on stop fills
